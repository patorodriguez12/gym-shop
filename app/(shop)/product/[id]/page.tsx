...
export default async function ProductPage({
  ...
  // 2. Obtenemos el usuario actual para la sincronización del carrito
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mt-4">
        ...
        <div className="relative aspect-square md:aspect-auto md:h-[500px] bg-gray-900 rounded-xl overflow-hidden border border-gray-800">
          {product.image_url ? (
            ...
            <div className="h-full flex items-center justify-center text-gray-600">Sin imagen</div>
          )}
        </div>
        ...
        <div className="flex flex-col">
          ...
          <h1 className="text-4xl font-bold mt-2 text-gray-900">{product.name}</h1>
          <p className="text-3xl font-bold mt-4 text-gray-800">${product.price}</p>

          <div className="my-6 border-t border-gray-800 pt-6">
            <h3 className="text-lg font-semibold mb-2 text-gray-600">Description</h3>
            <p className="text-gray-400 leading-relaxed text-gray-600">{product.description}</p>
          </div>
          ...
          <div className="mt-auto space-y-4">
            ...
            {/* 3. Reemplaza el botón estático por el componente AddToCartButton */}
            <AddToCartButton 
              product={product} 
              userId={user?.id} 
            />
